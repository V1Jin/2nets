...
def get_response():
    try:
        ...
        concerts_url = f'https://afisha.yandex.ru/krasnodar?rubric=concert&date={time}&period=1'
        ...
        print ("status concerts", response.status_code)
        ...
        print ("status sports", response.status_code)
        soup = BeautifulSoup(response.content, 'html.parser')
        events = soup.find_all("div", class_="event events-list__item yandex-sans")
        for i in range(len(events)):
            new_mass.append(make_jsons(events[i]))
